...
export function ReadingRoomClient({ document }: { document: any }) {
  ...
  return (
    <div className="container mx-auto py-8 px-4 lg:px-8 max-w-[1400px]">
      ...
      <div className="bg-slate-50/20 p-4 rounded-3xl">
        {/* Main Reading Area (Side by side) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Left Column: Original Text */}
          <Card className="border-slate-200 shadow-sm bg-slate-50 flex flex-col h-[calc(100vh-280px)]">
            <CardHeader className="border-b border-slate-200 bg-white px-6 py-4 flex-none rounded-t-xl">
              <CardTitle className="flex items-center gap-2 text-lg font-bold text-slate-800">
                <FileText className="h-5 w-5 text-slate-400" />
                Original Text
              </CardTitle>
            </CardHeader>
            <CardContent className="p-6 overflow-y-auto flex-1 font-mono text-sm leading-relaxed text-slate-600 whitespace-pre-wrap">
              {document.originalText}
            </CardContent>
          </Card>

          {/* Right Column: Simplified Translation */}
          <Card className="border-blue-100 shadow-sm bg-white flex flex-col h-[calc(100vh-280px)]">
            <CardHeader className="border-b border-blue-100 bg-blue-50/50 px-6 py-4 flex-none rounded-t-xl">
              <CardTitle className="flex items-center gap-2 text-lg font-bold text-blue-900">
                <BookOpen className="h-5 w-5 text-blue-600" />
                Plaintext Translation
              </CardTitle>
            </CardHeader>
            <CardContent className="p-8 overflow-y-auto flex-1 prose prose-slate prose-headings:font-bold prose-headings:text-slate-900 prose-p:leading-relaxed prose-a:text-blue-600 max-w-none">
              {activeSimplification ? (
                <ReactMarkdown>
                  {activeSimplification.simplifiedText}
                </ReactMarkdown>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {activeSimplification.glossaryTerms.map(
                  (term: { id: string; term: string; definition: string }) => (
                    <Card
                      key={term.id}
                      className="border-slate-200 bg-white hover:border-blue-200 transition-colors"
                    >
